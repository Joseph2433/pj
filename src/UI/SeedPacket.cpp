      ...
      m_position(position), m_size(size)
{
    ...
    float iconScaleY = (m_size.y * SEED_PACKET_ICON_SCALE_FACTOR * 0.7f) / iconLocalBounds.height;
    ...
    m_cooldownOverlay.setSize(sf::Vector2f(m_size.x, 0));

    // Cooldown Text (Optional)
    m_cooldownText.setFont(m_secondaryFontRef);
...
void SeedPacket::update(float dt, int currentSun, bool isExternallySelected)
{
    ...
    {
        m_cooldownOverlay.setSize(sf::Vector2f(0, 0));
...
void SeedPacket::updateAppearance(int currentSun)
{
    ...
    {
        bgColor = sf::Color(100, 150, 100, 200);
...
bool SeedPacket::isOnCooldown() const { return m_currentCooldown > 0.001f; }
